...
int str_compare(str_ptrs s1, str_ptrs s2) {
    ...
    for (int i = 0; i < len_longest_str; i++) {
        if ((s1.str_start + symbol_s1) != s1.str_end && (s2.str_start + symbol_s2) != s2.str_end) {
            while (!isalpha(s1.str_start[symbol_s1]) && (s1.str_start + symbol_s1) != s1.str_end) {
                symbol_s1++;
            }
            while (!isalpha(s2.str_start[symbol_s2]) && (s2.str_start + symbol_s2) != s2.str_end) {
                symbol_s2++;
            }
            if (tolower(s1.str_start[symbol_s1]) != tolower(s2.str_start[symbol_s2])) {
                return tolower(s1.str_start[symbol_s1]) - tolower(s2.str_start[symbol_s2]);
            }
            symbol_s1++;
            symbol_s2++;
        }
